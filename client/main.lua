...
local function smashVitrine(k)
    if validWeapon() then
        ...
        elseif math.random(1, 100) <= 5 and IsWearingHandshoes() then
            TriggerServerEvent("evidence:server:CreateFingerDrop", plyCoords)
            QBCore.Functions.Notify("Camın üzerinde parmak izi bıraktın.", "error")
        end
        smashing = true
        QBCore.Functions.Progressbar("smash_vitrine", "Looting case", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
            ...
            TriggerServerEvent('qb-jewellery:server:setVitrineState', "Açıldı", true, k)
            TriggerServerEvent('qb-jewellery:server:setVitrineState', "Mesgul", false, k)
            TriggerServerEvent('qb-jewellery:server:vitrineReward')
            TriggerServerEvent('qb-jewellery:server:setTimeout')
            TriggerServerEvent('police:server:policeAlert', 'Soygun Devam Ediyor')
            smashing = false
        ...
        QBCore.Functions.Notify('Silahınız Yeterince Güçlü Gözükmüyor..', 'error')
    end
end
        ...
        if result then 
            ...
            QBCore.Functions.TriggerCallback('qb-jewellery:server:getCops', function(cops)
                if cops >= Config.RequiredCops then
                    ...
                    function()
                        QBCore.Functions.Notify("Termit Başarısız Oldu..", "error")
                    end)
                else
            ...
            QBCore.Functions.Notify("Birşeyler Kaçırıyorsun..", "error", 2500)
        end
